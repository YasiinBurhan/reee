    ...
    private PackageInfo getPackageInfoWithFallback(String packageName, int userId) {
        try {
            ...
            try {
                ...
                try {
                    PackageInfo hostInfo = getPackageManager().getPackageInfo(packageName, 0);
                    if (hostInfo != null) {
                        Slog.d(TAG, "Obtained PackageInfo from host PackageManager for " + packageName);
                        return hostInfo;
                    }
                } catch (Exception e4) {
                    Slog.w(TAG, "Failed to get package info from host PackageManager for " + packageName + ": " + e4.getMessage());
                }
            }
        }
